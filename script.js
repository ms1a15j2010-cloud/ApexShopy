...
style.id = "apex-affiliate-button-styles";
...
function getApexCategory(product) {
...
These products remain visible in the
ApexShopy catalogue under Accessories.
*/
...
function getAffiliateLabel(product) {
const source =
String(
product?.source ||
""
...
function getAffiliateButton(product) {
if (
!hasAffiliateUrl(
product
)
...
container.innerHTML = `      <div class="no-results">         <p>
          ${
            searchQuery
...
let priceHTML = `     <div class="modal-price-row">
  `;
...
priceHTML += `       <span class="sale-price modal-price-usd">
        ${formatUSD(
          priceUSD
        )}       </span>
    `;
...
priceHTML += `       <span class="modal-price-pkr">
        ${formatPKR(
          pricePKR
        )}       </span>     </div>
  `;
...
modal.innerHTML = ` <div class="modal-content product-modal-content">
...
cartTotalAmountEl.innerHTML = `         ${formatUSD(
          totalPriceUSD
        )}         <span class="price-pkr">
          ${formatPKR(
            totalPricePKR
...
cartItemsEl.innerHTML = `       <p class="empty-cart-msg">
